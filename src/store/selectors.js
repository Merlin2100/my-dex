...
let token0Amount, token1Amount
    ...
    if (order._tokenGive === tokens[0].address) {
        token0Amount = order._amountGive // The amount og MT we're giving
    ...
    return ({
        ...
        _fromattedTimestamp: moment.unix(order._timestamp).format()
    })
 }
...
export const orderBookSelector = createSelector(openOrders, tokens, (orders, tokens) => {
    ...
    // Sort buy order by token price (from highest price to lowest price)
    orders = {
        ...orders,
    ...
    // Sort sell order by token price (from highest price to lowest price)
    orders = {
        ...orders,
